...
if __package__:
    ...
    from parents.rel import EvidenceRecord, ConsistencyRelation, RelationalEvidenceLocalizer
if __package__:
    from .parents.ebc import HistoricalEstimate, BorrowingResult, borrow_evidence
else:
    from parents.ebc import HistoricalEstimate, BorrowingResult, borrow_evidence
...
def gate_and_borrow(
    *,
    ...
    localized = localizer.localize(records, relations, max_colluding_records=max_colluding_records)
    ...
    for item in history:
        if active:
            p = min(1.0, max(0.0, float(marginals.get(item.name, 0.0))))
        ...
        gated_history.append(HistoricalEstimate(item.name, item.estimate, item.standard_error, gated_power))
    ...
    return IntegrityBorrowingResult(
        ...
        posterior_semantics=(
            "REL marginal suspect probabilities are conditional on the enumerated one-or-more-culprit hypothesis space; "
