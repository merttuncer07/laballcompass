...
def catalog():
 build();return json.loads((BASE/'catalog.json').read_text())['components']
def resolve(query):
 rows=catalog();exact=[r for r in rows if r['id']==query]
...
def main(args):
 p=argparse.ArgumentParser();p.add_argument('action',choices=['components','show','check','demo']);p.add_argument('id',nargs='?');p.add_argument('--query',default='');p.add_argument('--entry');p.add_argument('--timeout',type=float,default=180);a=p.parse_args(args)
 try:
  if a.action=='components':
   for r in catalog():
    if a.query.lower() in json.dumps(r).lower():print(f"{r['id']:38} {r['layer']:16} {r['latest_reproduction']['status']}")
   return 0
  ...
  if a.action=='check':
   ...
   path=run(targets,profile='component:'+row['id'],timeout=a.timeout);print(path);return int(json.loads(path.read_text())['status']!='PASS')
