...
# ---- LCB-7F3A91 value-first: RH-010 ----
class ObservationPolicyConfoundingGuardV0:
    """Use observation-policy metadata jointly with a passive state proxy before estimating measurement backaction."""
    @staticmethod
    def posterior_state_grid(passive_proxy, observed, state_grid, posterior_mean_if_unobserved,
                             posterior_mean_if_observed, proxy_grid):
        import numpy as np
        c=np.asarray(passive_proxy,float); o=np.asarray(observed,int)
        pg=np.asarray(proxy_grid,float)
        p0=np.asarray(posterior_mean_if_unobserved,float)
        p1=np.asarray(posterior_mean_if_observed,float)
        out=np.empty_like(c)
        m=o.astype(bool)
        out[~m]=np.interp(c[~m],pg,p0)
        out[m]=np.interp(c[m],pg,p1)
        return out
    @staticmethod
    def adjusted_backaction(next_state, observed, posterior_state):
        import numpy as np
        y=np.asarray(next_state,float); o=np.asarray(observed,float); s=np.asarray(posterior_state,float)
        X=np.column_stack([np.ones(len(y)),o,s])
        b=np.linalg.lstsq(X,y,rcond=None)[0]
        return {"backaction_coefficient":float(b[1]),"state_coefficient":float(b[2])}

# ---- LCB-7F3A91 value-first: RX-020 ----
class MicroMemoryDiagnosticV0:
    """Measure directional transition memory that can survive cancellation of the macro projection."""
    @staticmethod
    def directional_autocorrelation(direction_sequences):
        import numpy as np
        X=np.asarray(direction_sequences,float)
        if X.ndim!=2 or X.shape[1]<2:
            raise ValueError("trajectories x time matrix required")
        return np.mean(X[:,:-1]*X[:,1:],axis=1)
    @staticmethod
    def population_summary(direction_sequences):
        import numpy as np
        X=np.asarray(direction_sequences,float)
        ac=MicroMemoryDiagnosticV0.directional_autocorrelation(X)
        return {"signed_macro_projection":float(X.mean()),
                "mean_absolute_trajectory_projection":float(np.mean(np.abs(X.mean(axis=1)))),
                "mean_directional_autocorrelation":float(np.mean(ac))}

# ---- LCB-7F3A91 merge extension: RX-118 -> ObservationTimingSensorV0 ----
def _event_innovation(count, expected_intensity):
    import numpy as np
    c=np.asarray(count,float); lam=np.asarray(expected_intensity,float)
    z=(c-lam)/np.sqrt(np.maximum(lam,1e-12))
    return {"innovation":z,"mean_innovation":float(np.mean(z))}
ObservationTimingSensorV0.event_innovation=staticmethod(_event_innovation)

# ---- LCB-7F3A91 merge extension: RX-090 -> ResilientFlowRouterV0 ----
def _load_feasibility(graph, source, sink, demand, capacity="capacity"):
    import networkx as nx
    if not nx.has_path(graph,source,sink):
        return {"physically_connected":False,"max_flow":0.0,"demand":float(demand),"usable":False}
    mf=float(nx.maximum_flow_value(graph,source,sink,capacity=capacity))
    return {"physically_connected":True,"max_flow":mf,"demand":float(demand),"usable":bool(mf>=float(demand))}
ResilientFlowRouterV0.load_feasibility=staticmethod(_load_feasibility)


# ---- LCB-7F3A91 value-first: RX-106 ----
class CodedBottleneckRouterV0:
    """Exploit receiver side information with a linear/XOR mixture across a narrow interface."""
    @staticmethod
    def two_packet_xor(packet_a, packet_b):
        if isinstance(packet_a,(bytes,bytearray)) and isinstance(packet_b,(bytes,bytearray)):
            if len(packet_a)!=len(packet_b):
                raise ValueError("byte packets must have equal length")
            return bytes(a ^ b for a,b in zip(packet_a,packet_b))
        return packet_a ^ packet_b
    @staticmethod
    def recover_from_xor(coded_packet, side_information):
        return CodedBottleneckRouterV0.two_packet_xor(coded_packet,side_information)

# ---- LCB-7F3A91 value-first: RX-121 ----
class DecisionWeightedCalibrationV0:
    """Allocate finite calibration samples by downstream consequence sensitivity."""
    @staticmethod
    def allocate(total_budget, noise_scale, consequence_weight, min_per_channel=2):
        import numpy as np
        sig=np.asarray(noise_scale,float)
        w=np.asarray(consequence_weight,float)
        if sig.shape!=w.shape:
            raise ValueError("noise_scale and consequence_weight must align")
        a=np.abs(w)*sig
        if np.all(a==0):
            a=np.ones_like(a)
        raw=np.maximum(int(min_per_channel),
                       np.floor(float(total_budget)*a/a.sum()).astype(int))
        while raw.sum()<int(total_budget):
            raw[int(np.argmax(a/np.sqrt(raw)))] += 1
        while raw.sum()>int(total_budget):
            candidates=np.where(raw>int(min_per_channel))[0]
            if not len(candidates): break
            j=candidates[np.argmax(raw[candidates])]
            raw[j]-=1
        return {"samples":raw,
                "total":int(raw.sum()),
                "criterion":"minimize downstream weighted calibration variance"}

# ---- LCB-7F3A91 value-first: RX-161 ----
class IrreversibleExplorationBudgetV0:
    """Distribution-gated continuation threshold for irreversible sequential selection."""
    @staticmethod
    def normal_continuation_threshold(history, remaining):
        import numpy as np
        from scipy.stats import norm
        h=np.asarray(history,float)
        if len(h)<2:
            raise ValueError("at least two observations required")
        mu=float(h.mean()); sd=float(h.std(ddof=1)+1e-12)
        r=max(int(remaining),2)
        return float(mu+sd*norm.ppf(1-1/r))
    @staticmethod
    def should_commit(current_value, history, remaining, normal_shell_valid=True):
        if not normal_shell_valid:
            return {"accepted":False,
                    "reason":"continuation distribution shell not certified; use rank/record fallback"}
        th=IrreversibleExplorationBudgetV0.normal_continuation_threshold(history,remaining)
        return {"accepted":bool(float(current_value)>=th),"threshold":th}

# ---- LCB-7F3A91 value-first: RX-183 ----
class TestingWealthControllerV0:
    """Finite-horizon LORD-like online testing budget that recycles alpha after discoveries."""
    def __init__(self, horizon, alpha=0.05, gamma_power=1.6):
        import numpy as np
        self.horizon=int(horizon)
        self.alpha=float(alpha)
        g=np.arange(1,self.horizon+1,dtype=float)**(-float(gamma_power))
        self.gamma=g/g.sum()
        self.rejections=[]
        self.t=0
    def next_alpha(self):
        if self.t>=self.horizon:
            raise IndexError("testing horizon exhausted")
        a=self.alpha*self.gamma[self.t]
        for r in self.rejections:
            lag=self.t-r-1
            if lag>=0:
                a+=self.alpha*self.gamma[lag]
        return float(min(a,self.alpha))
    def test(self,p_value):
        a=self.next_alpha()
        reject=bool(float(p_value)<=a)
        if reject:
            self.rejections.append(self.t)
        result={"t":self.t,"alpha_t":a,"reject":reject,
                "discoveries_so_far":len(self.rejections)}
        self.t+=1
        return result


# ---- LCB-7F3A91 value-first: RX-154 ----
class DecisionSufficientCompressorV0:
    """Quantize a decision-relevant scalar score under a fixed code budget."""
    @staticmethod
    def fit(score, target, bits=4):
        import numpy as np
        s=np.asarray(score,float); y=np.asarray(target,float)
        bins=2**int(bits)
        edges=np.quantile(s,np.linspace(0,1,bins+1))
        edges[0]-=1e-12; edges[-1]+=1e-12
        idx=np.clip(np.digitize(s,edges)-1,0,bins-1)
        prob=np.array([y[idx==j].mean() if np.any(idx==j) else y.mean() for j in range(bins)])
        return {"edges":edges,"bin_probability":prob,"bits":int(bits)}
    @staticmethod
    def encode(score, model):
        import numpy as np
        s=np.asarray(score,float)
        edges=np.asarray(model["edges"],float)
        bins=len(edges)-1
        return np.clip(np.digitize(s,edges)-1,0,bins-1)
    @staticmethod
    def predict(score, model):
        import numpy as np
        idx=DecisionSufficientCompressorV0.encode(score,model)
        return np.asarray(model["bin_probability"])[idx]

# ---- LCB-7F3A91 value-first: RX-107 ----
class CandidateSetDecoderV0:
    """Return the smallest likelihood/posterior candidate list reaching a requested mass."""
    @staticmethod
    def posterior_list(log_likelihood, mass=0.95):
        import numpy as np
        ll=np.asarray(log_likelihood,float)
        if ll.ndim!=1:
            raise ValueError("one log-likelihood per candidate required")
        w=np.exp(ll-np.max(ll))
        p=w/w.sum()
        order=np.argsort(-p)
        cum=np.cumsum(p[order])
        k=int(np.searchsorted(cum,float(mass),side="left")+1)
        keep=order[:k]
        return {"candidate_indices":keep,
                "candidate_probabilities":p[keep],
                "list_size":k,
                "covered_mass":float(cum[k-1]),
                "top1_index":int(order[0])}

# ---- LCB-7F3A91 merge extension: RX-061 -> SolverCertificateAuditV0 ----
def _validate_local_optimality_preconditions(active_constraint_gradients,
                                             objective_hessian_eigenvalues,
                                             gradient_tolerance=1e-8):
    import numpy as np
    G=np.asarray(active_constraint_gradients,float)
    eig=np.asarray(objective_hessian_eigenvalues,float)
    if G.size==0:
        cq_ok=True
        min_active_grad=float("inf")
    else:
        if G.ndim==1:
            G=G.reshape(1,-1)
        rownorm=np.linalg.norm(G,axis=1)
        min_active_grad=float(np.min(rownorm))
        cq_ok=bool(min_active_grad>float(gradient_tolerance))
    curvature_ok=bool(np.min(eig)>=-float(gradient_tolerance)) if eig.size else True
    return {"constraint_geometry_ok":cq_ok,
            "minimum_active_gradient_norm":min_active_grad,
            "curvature_or_convexity_ok":curvature_ok,
            "preconditions_ok":bool(cq_ok and curvature_ok)}
SolverCertificateAuditV0.validate_local_optimality_preconditions=staticmethod(
    _validate_local_optimality_preconditions
)

# ---- LCB-7F3A91 merge extension: RH-002 -> CausalEdgeValidatorFeaturesV0 ----
def _environment_invariance_rank(source_errors, target_errors):
    import numpy as np
    s=np.asarray(source_errors,float)
    t=np.asarray(target_errors,float)
    if s.shape!=t.shape:
        raise ValueError("source and target candidate errors must align")
    drift=np.abs(t-s)
    return {"invariance_drift":drift,
            "best_candidate_index":int(np.argmin(drift))}
CausalEdgeValidatorFeaturesV0.environment_invariance_rank=staticmethod(
    _environment_invariance_rank
)


# ---- LCB-7F3A91 value-first: RX-196 ----
class SelectionAwareReferenceLawV0:
    """Simultaneous/selection-aware interval for a data-selected maximum estimate."""
    @staticmethod
    def sidak_selected_interval(estimate, standard_error, family_size, confidence=0.95):
        from scipy.stats import norm
        K=max(int(family_size),1)
        c=float(confidence)**(1.0/K)
        z=float(norm.ppf((1.0+c)/2.0))
        e=float(estimate); se=float(standard_error)
        return {"lower":e-z*se,"upper":e+z*se,
                "critical_value":z,
                "family_size":K,
                "confidence":float(confidence)}

# ---- LCB-7F3A91 value-first: RX-097 ----
class ObservabilityCoveragePlannerV0:
    """Greedily select measurements to increase state-space rank and conditioning."""
    @staticmethod
    def select(sensor_rows, budget):
        ...
        for _ in range(min(int(budget),len(A))):
            ...
            for j in remaining:
                ...
                minsv=float(s[rank-1]) if rank>0 else 0.0
                score=(rank,minsv)
...
class ReachAvoidViabilityPlannerV0:
    """Validate path segments against circular avoid regions and choose a safe detour."""
    @staticmethod
    def segment_clearance(p,q,center,radius):
        import numpy as np
        p=np.asarray(p,float); q=np.asarray(q,float); c=np.asarray(center,float)
        v=q-p
        vv=float(v@v)
        if vv==0:
            return float(np.linalg.norm(p-c)-float(radius))
        t=float(np.clip(((c-p)@v)/vv,0,1))
        closest=p+t*v
        return float(np.linalg.norm(closest-c)-float(radius))
    @staticmethod
    def path_safe(path, obstacles):
        min_clear=float("inf")
        for a,b in zip(path[:-1],path[1:]):
            for center,radius in obstacles:
                cl=ReachAvoidViabilityPlannerV0.segment_clearance(a,b,center,radius)
                min_clear=min(min_clear,cl)
                if cl<=0:
                    return {"safe":False,"minimum_clearance":float(cl)}
        return {"safe":True,"minimum_clearance":float(min_clear)}

# ---- LCB-7F3A91 merge extension: RX-155 -> DecisionSufficientCompressorV0 ----
def _minimum_bits_for_decision_tolerance(bit_error_curve, error_tolerance):
    items=sorted((int(k),float(v)) for k,v in dict(bit_error_curve).items())
    for bits,err in items:
        if err<=float(error_tolerance):
            return {"minimum_bits":bits,"achieved_error":err,"met_tolerance":True}
    return {"minimum_bits":None,"achieved_error":None,"met_tolerance":False}
DecisionSufficientCompressorV0.minimum_bits_for_decision_tolerance=staticmethod(
    _minimum_bits_for_decision_tolerance
)
