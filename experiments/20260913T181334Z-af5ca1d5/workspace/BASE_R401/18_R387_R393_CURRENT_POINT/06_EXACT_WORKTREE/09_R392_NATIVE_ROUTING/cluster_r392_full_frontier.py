...
LAB=HERE.parent
CORE=LAB/'01_V2_CORE'
ROUTING=json.loads((HERE/'R392_FULL_FAILURE_ROUTING.json').read_text())
...
assert authority['authority_release']=='R391', authority.get('authority_release')
...
for shadow_root in sorted(LAB.glob('*_NATIVE_ROUTING/SHADOW_PRODUCTS')):
    ...
    for d in sorted(shadow_root.glob('V2P*')):
        ...
        products_by_id[pid]={
            ...
            'family':family_by_pid.get(pid,'R392_SHADOW_DISTINCT_CANDIDATE'),
...
clusters=[]
for c in sorted(prepared,key=lambda z:z['r392_failure_rank']):
    ...
    for cl in clusters:
        ...
        if jac>=0.62 and (endpoint_same or (family_same and origin_same)):
            assigned=cl; break
    if assigned is None:
        clusters.append({'cluster_id':f'R392C{len(clusters)+1:04d}','rep':c,'members':[c]})
    else:
        assigned['members'].append(c)
for cl in clusters:
    cl['size']=len(cl['members']); cl['best_failure_rank']=min(x['r392_failure_rank'] for x in cl['members'])
...
for cl in sorted(clusters,key=lambda x:x['best_failure_rank']):
    ...
    summary.append({
      ...
      'member_ranks':[x['r392_failure_rank'] for x in cl['members'][:60]],
    })

out={
 'release':'R392','base_authority_release':'R391','edge_count':len(prepared),'cluster_count':len(clusters),
...
(HERE/'R392_FULL_CLUSTER_SCREEN.json').write_text(json.dumps(out,indent=2,ensure_ascii=False))
