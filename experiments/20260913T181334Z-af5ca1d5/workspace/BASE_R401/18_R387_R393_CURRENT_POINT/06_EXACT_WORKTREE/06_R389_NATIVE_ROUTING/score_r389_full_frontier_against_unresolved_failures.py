...
CANDS=HERE/'R389_FULL_FRESH_ADJUDICATION_QUEUE.jsonl'
...
def main():
    ...
    for i,e in enumerate(scored,1): e['r389_failure_rank']=i
    for e in scored[:500]:
        reps=[]
    ...
    (HERE/'R389_FULL_FAILURE_ROUTING.json').write_text(json.dumps(out,indent=2,ensure_ascii=False))
    print(json.dumps({'summary':out['summary'],'top_edges':[{
      'rank':e['r389_failure_rank'],'supplier':e['supplier_id'],'consumer':e['consumer_id'],'failure_relevance_score':e['failure_relevance_score'],
