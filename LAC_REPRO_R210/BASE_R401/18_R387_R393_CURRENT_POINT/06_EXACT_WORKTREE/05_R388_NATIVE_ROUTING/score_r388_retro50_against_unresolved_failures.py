...
BOUND=HERE/'R77_BOUNDARY_SENTENCES_SOURCE.json'
CANDS=HERE/'R388_RETRO50_FRESH_ADJUDICATION_QUEUE.jsonl'
...
def main():
    ...
    for e in cands:
        ...
        retro=math.log1p(rs+rc)*8 + math.log1p(hs+hc)*4 + min(category_div,6)*1.5 + native*0.15
        row=dict(e)
        row.update({
            'native_score':native,'failure_relevance_score':retro,'endpoint_relevance':rs+rc,
    ...
    for i,e in enumerate(scored,1): e['r388_failure_rank']=i
    for e in scored[:500]:
        reps=[]
    ...
    (HERE/'R388_RETRO50_FAILURE_ROUTING.json').write_text(json.dumps(out,indent=2,ensure_ascii=False))
    print(json.dumps({'summary':out['summary'],'top_edges':[{
      'rank':e['r388_failure_rank'],'supplier':e['supplier_id'],'consumer':e['consumer_id'],'failure_relevance_score':e['failure_relevance_score'],
      'native_score':e['native_score'],'origin':e['candidate_origin'],'interfaces':e['interface_matches'],'addressed':e['addressed_weaknesses'],
