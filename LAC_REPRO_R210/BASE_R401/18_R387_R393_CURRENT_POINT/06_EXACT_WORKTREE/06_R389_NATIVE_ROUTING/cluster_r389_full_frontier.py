...
ROUTING=json.loads((HERE/'R389_FULL_FAILURE_ROUTING.json').read_text())
...
assert authority['authority_release']=='R388'
canonical=set(authority['canonical_executable_suite_ids'])
quarantined=set(authority.get('quarantined_product_ids',[]))

family_by_pid={}
for fn in ('PRODUCT_QUALITY_AUDIT_V1.json','PRODUCT_QUALITY_AUDIT_R386.json','PRODUCT_QUALITY_AUDIT_R388.json'):
    p=CORE/fn
...
if shadow_root.exists():
    for d in sorted(shadow_root.glob('V2P*')):
        ...
        products_by_id[pid]={'id':pid,'dir':'05_R388_NATIVE_ROUTING/SHADOW_PRODUCTS/'+d.name,'text':'\n'.join(chunks),'family':'R389_SHADOW_DISTINCT_CANDIDATE','canonical':False,'quarantined':False}
...
clusters=[]
for c in sorted(prepared,key=lambda z:z['r389_failure_rank']):
    ...
    for cl in clusters:
        ...
        if jac>=0.62 and (endpoint_same or (family_same and origin_same)):
            assigned=cl; break
    if assigned is None:
        clusters.append({'cluster_id':f'R389C{len(clusters)+1:04d}','rep':c,'members':[c]})
    else:
        assigned['members'].append(c)
for cl in clusters:
    cl['size']=len(cl['members']); cl['best_failure_rank']=min(x['r389_failure_rank'] for x in cl['members'])
...
for cl in sorted(clusters,key=lambda x:x['best_failure_rank']):
    ...
    summary.append({
      ...
      'member_ranks':[x['r389_failure_rank'] for x in cl['members'][:60]],
    })

out={'release':'R389','base_authority_release':'R388','edge_count':len(prepared),'cluster_count':len(clusters),'product_count_for_collision':len(products),'canonical_products_for_collision':len(canonical),'quarantined_products_for_collision':len(quarantined),'shadow_products_for_collision':sum(not p['canonical'] and not p['quarantined'] for p in products),'clusters':summary,'diverse_selected':selected,'nonclaim':'lexical/mechanism clustering is workload triage only; distinct-family status still requires code-level non-additivity and mechanism-removing control'}
(HERE/'R389_FULL_CLUSTER_SCREEN.json').write_text(json.dumps(out,indent=2,ensure_ascii=False))
print(json.dumps({'edges':len(prepared),'clusters':len(clusters),'products':len(products),'canonical':len(canonical),'top_diverse':[{
 'selection_rank':i+1,'cluster':c['cluster_id'],'supplier':c['supplier_id'],'consumer':c['consumer_id'],'diverse_score':round(c['diverse_score'],3),'failure_rank':c['r389_failure_rank'],'origin':c.get('visibility_origin'),'nearest':c['nearest_product'],'family':c['nearest_quality_family'],'collision':round(c['product_collision'],3),'interfaces':c['interface_matches'],'addressed':c['addressed_weaknesses']} for i,c in enumerate(selected[:50])]},indent=2,ensure_ascii=False))
