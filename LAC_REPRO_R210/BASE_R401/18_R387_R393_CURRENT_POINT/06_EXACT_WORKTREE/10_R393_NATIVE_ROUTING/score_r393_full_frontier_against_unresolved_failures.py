...
CANDS=HERE/'R393_FULL_FRESH_ADJUDICATION_QUEUE.jsonl'
...
def main():
    ...
    for i,e in enumerate(scored,1): e['r393_failure_rank']=i
    for e in scored[:500]:
        reps=[]
    ...
    (HERE/'R393_FULL_FAILURE_ROUTING.json').write_text(json.dumps(out,indent=2,ensure_ascii=False))
    print(json.dumps({'summary':out['summary'],'top_edges':[{
      'rank':e['r393_failure_rank'],'supplier':e['supplier_id'],'consumer':e['consumer_id'],'failure_relevance_score':e['failure_relevance_score'],
