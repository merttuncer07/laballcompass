...
PAIR=('FOUNDRY:P090','PARENT:R041_CONSTRAINED_POLICY_CAPL'); PID='V2P053'
...
if any(e.event_id.startswith('R392::V2P053::') for e in old.events): raise RuntimeError('R392 V2P053 pilot already present')
...
draft=bootstrap_queue_row(row,root=CORE,search_generation=392)
if draft.suite is None or draft.suite.product_id!=PID: raise RuntimeError('bootstrap did not resolve V2P053')
events=run_fixed_suite_pilot(draft,release_generation=392,timeout_seconds=60.,event_namespace='R392')
...
tmp=ledger_path.with_suffix('.r392.tmp'); save_ledger(tmp,merged); os.replace(tmp,ledger_path)
receipt={'release':'R392','product_id':PID,'pair':list(PAIR),'events_added':len(events),'old_event_count':len(old.events),'new_event_count':len(merged.events),'statuses':{s:sum(e.run_status==s for e in events) for s in ('COMPLETE','FAILED','CENSORED')},'test_signature':suite.test_signature,'problem_shell':suite.problem_shell,'cases':list(suite.cases),'evidence_boundary':'mechanics/executable-contract calibration only; not empirical validation'}
(ROOT/'09_R392_NATIVE_ROUTING/R392_V2P053_RAPID_PILOT.json').write_text(json.dumps(receipt,indent=2,sort_keys=True)+'\n')
